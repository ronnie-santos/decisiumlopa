"""
contaspagar_report.py — Relatório de Contas a Pagar
Herda BaseReport para layout padrão (cabeçalho, rodapé).
Layout: retrato A4, 7 colunas, com suporte a agrupamento por fornecedor ou empresa.
"""
...
COLOR_SUBTOTAL  = colors.HexColor("#E2E8F0")   # slate-200 — subtotal
COLOR_TOTAL_BG  = colors.HexColor("#111827")   # slate-900 — total geral
COLOR_ABERTA    = colors.HexColor("#FFFBEB")   # amber-50  — conta em aberto
COLOR_QUITADA   = colors.HexColor("#F8FAFC")   # slate-50  — conta quitada
...
# ── Relatório ─────────────────────────────────────────────────────────────────
class ContasPagarReport(BaseReport):
    """
    Relatório de Contas a Pagar.
    Layout retrato A4, com suporte a agrupamento por fornecedor ou empresa.
    """

    COLS = ["Vencimento", "Fornecedor", "Nota", "Valor", "Empresa", "Valor Pago", "Situação"]
    N_COLS = len(COLS)
    ...
    # ── Larguras das colunas (retrato A4) ─────────────────────────────────────
    ...
    def _row_from(self, row: dict) -> list:
        ...
        nota_str = f"{nota}" + (f" / {parcela}" if parcela else "")
    ...
    def _build_flat_table(self, rows: list[dict], col_widths: list[float]) -> Table:
        ...
        style_cmds = [
            ...
            ("ALIGN",         (5, 1), (5, -1), "RIGHT"),   # Valor Pago
        ]
    ...
    def generate(
        ...
        grupo_labels = {1: "Normal", 2: "por Fornecedor", 3: "por Empresa"}
        ...
        if total_registros == 0:
            elements.append(Paragraph("Nenhuma conta encontrada para o período e filtros selecionados.", periodo_style))
        elif grupo_tipo == 1:
