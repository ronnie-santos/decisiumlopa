"""
import_cliente.py — Importa tabela 'cliente' a partir de dump Progress 9.1E (.d)

Colunas esperadas (na ordem do dump) — 10 campos:
  idcliente, nome, nomefantasia, data_cadastro, cnpj_cpf, ie_rg,
  tipo, observacao, site, contato

Nota: status não está no dump; definido como 'ATIVO' por padrão.

Uso:
    python import_cliente.py
    python import_cliente.py caminho/para/cliente.d
    python import_cliente.py cliente.d --dry-run
"""
...
def read_d_file(path: str) -> list[dict]:
    ...
    for lineno, raw in enumerate(merged, start=1):
        ...
        (
            idcliente, nome, nomefantasia, data_cadastro,
            cnpj_cpf, ie_rg, tipo, observacao, site, contato
        ) = tokens[:EXPECTED_COLS]

        if idcliente is None:
            print(f"  [AVISO] Linha {lineno} ignorada (idcliente vazio): {raw.rstrip()[:80]}")
            skipped += 1
            continue
...
UPSERT_SQL = """
    INSERT INTO cliente (
        idcliente, nome, nomefantasia, data_cadastro,
        cnpj_cpf, ie_rg, tipo, observacao, site, contato, status
    )
    VALUES %s
...
def import_records(records: list[dict], dry_run: bool = False):
    values = [
        (
            r["idcliente"], r["nome"], r["nomefantasia"], r["data_cadastro"],
            r["cnpj_cpf"], r["ie_rg"], r["tipo"], r["observacao"],
            r["site"], r["contato"], r["status"],
        )
        for r in records
    ]
    ...
    if dry_run:
        ...
        for v in values[:20]:
            print(f"  id={str(v[0]):>5}  nome={str(v[1])[:40]:<40}  cnpj={v[4]}")
        if len(values) > 20:
...
def main():
    parser = argparse.ArgumentParser(
        description="Importa cliente.d (Progress 9.1E) → PostgreSQL (upsert por idcliente)"
    )
    parser.add_argument(
        "arquivo",
        nargs="?",
        default=os.path.join(os.path.dirname(__file__), "EXTRACAO", "cliente.d"),
        help="Caminho para o arquivo .d (padrão: EXTRACAO/cliente.d)",
    )
