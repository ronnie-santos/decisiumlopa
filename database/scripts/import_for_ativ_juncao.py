"""
import_for_ativ_juncao.py — Importa junção fornecedor↔atividade a partir de dump Progress 9.1E (.d)

Fonte: for_ativ.d
Formato: idfornecedor idatividade (2 campos por linha)
Destino: tabela 'for_atividade' (serial PK idforatividade, idfornecedor, idatividade)

Nota: ON CONFLICT (idfornecedor, idatividade) DO NOTHING.
      Constraint UNIQUE criada automaticamente se não existir.

Uso:
    python import_for_ativ_juncao.py
    python import_for_ativ_juncao.py caminho/para/for_ativ.d
    python import_for_ativ_juncao.py for_ativ.d --dry-run
"""
...
def ensure_unique_constraint(conn):
    with conn.cursor() as cur:
        cur.execute("""
            ...
            WHERE table_name = 'for_atividade'
              AND constraint_name = 'uq_for_atividade_fornecedor_atividade'
        """)
        if not cur.fetchone():
            cur.execute("""
                ALTER TABLE for_atividade
                ADD CONSTRAINT uq_for_atividade_fornecedor_atividade
                UNIQUE (idfornecedor, idatividade)
            """)
            print("  Constraint UNIQUE criada em for_atividade(idfornecedor, idatividade).")
...
def read_d_file(path):
    ...
    with open(path, encoding=FILE_ENCODING, errors="replace") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            ativ = to_int(tokens[1])
            if forn is None or ativ is None:
                skipped += 1
                continue
            records.append({"idfornecedor": forn, "idatividade": ativ})
    if skipped:
...
UPSERT_SQL = """
    INSERT INTO for_atividade (idfornecedor, idatividade)
    VALUES %s
    ON CONFLICT (idfornecedor, idatividade) DO NOTHING
"""
...
def import_records(records, dry_run=False):
    ...
    deduped = []
    for r in records:
        key = (r["idfornecedor"], r["idatividade"])
        if key not in seen:
            seen.add(key)
            deduped.append(r)
    ...
    if dry_run:
        values = [(r["idfornecedor"], r["idatividade"]) for r in deduped]
        print(f"\n[DRY-RUN] {len(values)} par(es) seriam importados:")
        for v in values[:20]:
            print(f"  idfornecedor={v[0]:>5}  idatividade={v[1]:>5}")
        if len(values) > 20:
    ...
    try:
        with conn:
            ...
            values = [(r["idfornecedor"], r["idatividade"]) for r in filtered]
            with conn.cursor() as cur:
...
def main():
    parser = argparse.ArgumentParser(
        description="Importa for_ativ.d (Progress 9.1E) → PostgreSQL for_atividade (junção)"
    )
    parser.add_argument(
        "arquivo",
        nargs="?",
        default=os.path.join(os.path.dirname(__file__), "EXTRACAO", "for_ativ.d"),
    )
