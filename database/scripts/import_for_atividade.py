"""
import_for_atividade.py — Importa tabela 'fornecedor_atividade' a partir de dump Progress 9.1E (.d)

Colunas esperadas (na ordem do dump):
  idatividade, descricao

Uso:
    python import_for_atividade.py
    python import_for_atividade.py caminho/para/for_ati.d
    python import_for_atividade.py for_ati.d --dry-run
"""
...
def read_d_file(path: str) -> list[dict]:
    ...
    with open(path, encoding=FILE_ENCODING, errors="replace") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            idatividade, descricao = tokens[:EXPECTED_COLS]

            if not idatividade:
                print(f"  [AVISO] Linha {lineno} ignorada (idatividade vazio): {raw.rstrip()}")
                skipped += 1
                continue
...
UPSERT_SQL = """
    INSERT INTO fornecedor_atividade (idatividade, descricao)
    VALUES %s
    ON CONFLICT (idatividade) DO UPDATE SET
        descricao = EXCLUDED.descricao
"""


def import_records(records: list[dict], dry_run: bool = False):
    values = [(r["idatividade"], r["descricao"]) for r in records]

    if dry_run:
        print(f"\n[DRY-RUN] {len(values)} registro(s) seriam importados:")
        for v in values[:20]:
            print(f"  idatividade={v[0]:>4}  descricao={str(v[1])[:60]}")
        if len(values) > 20:
...
def main():
    parser = argparse.ArgumentParser(
        description="Importa for_ati.d (Progress 9.1E) → PostgreSQL (upsert por idatividade)"
    )
    parser.add_argument(
        "arquivo",
        nargs="?",
        default=os.path.join(os.path.dirname(__file__), "EXTRACAO", "for_ati.d"),
        help="Caminho para o arquivo .d (padrão: EXTRACAO/for_ati.d na mesma pasta do script)",
    )
