"""
import_estado.py — Importa tabela 'estados' a partir de dump Progress 9.1E (.d)
...
Colunas esperadas (na ordem do dump): idestado, nome, idpais

Uso:
    python import_estado.py                           # usa estados.d na pasta extracao/
    python import_estado.py caminho/para/estados.d
    python import_estado.py estados.d --dry-run       # apenas exibe sem gravar
"""
...
def read_d_file(path: str) -> list[dict]:
    ...
    with open(path, encoding=FILE_ENCODING, errors="replace") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            idestado, nome, idpais = tokens[:3]

            if not idestado:
                print(f"  [AVISO] Linha {lineno} ignorada (idestado vazio): {raw.rstrip()}")
                skipped += 1
                continue
...
# ── Importação ────────────────────────────────────────────────────────────────
INSERT_SQL = """
    INSERT INTO estados (idestado, nome, idpais)
    VALUES %s
"""
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("SELECT idestado FROM estados")
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Importa estados.d (Progress 9.1E) → PostgreSQL")
    parser.add_argument(
        "arquivo",
        nargs="?",
        default=os.path.join(os.path.dirname(__file__), "extracao/estados.d"),
        help="Caminho para o arquivo .d (padrão: estados.d na pasta extracao/)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
