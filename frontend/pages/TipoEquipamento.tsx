...
export function TipoEquipamentoPage() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<TipoEquipamento | null>(null);
  const [items, setItems] = useState<TipoEquipamento[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<TipoEquipamento | null>(null);
  ...
  const filteredItems = items
    ...
    .sort((a, b) => Number(a.idtipoequipamento) - Number(b.idtipoequipamento));

  const cols: GridColumn<TipoEquipamento>[] = [
    { header: 'Código', render: i => <span className="text-xs font-bold text-[#B21212]">{i.idtipoequipamento}</span> },
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch('/api/tipo-equipamento');
      if (response.ok) {
        const data = await response.json();
        setItems(data.map((d: any) => ({ ...d, id: String(d.idtipoequipamento) })));
      } else { console.error('Falha ao carregar tipos de equipamento'); }
    } catch (error) { console.error('Erro na requisição:', error); }
  };

  const handleOpenNewModal = () => { setEditingItem(null); setFormData({ nome: '' }); setIsModalOpen(true); };

  const handleOpenEditModal = (item: TipoEquipamento) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isEditing ? `/api/tipo-equipamento/${editingItem!.idtipoequipamento}` : '/api/tipo-equipamento';
  ...
  const handleDeleteRequest = (item: TipoEquipamento) => { setItemToDelete(item); setDeleteError(null); setIsDeleteModalOpen(true); };

  const handleConfirmDelete = async () => {
    if (!itemToDelete) return;
    try {
      const response = await fetch(`/api/tipo-equipamento/${itemToDelete.idtipoequipamento}`, { method: 'DELETE' });
  ...
  return (
    <div className="flex flex-col h-full">
      <Header title="Tipos de Equipamentos" />
      <div className="p-5 space-y-4">
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="flex-1 min-w-[200px]">
              <input className="w-full border border-slate-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-200" placeholder="Buscar tipo de equipamento..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
        ...
        <DataGrid data={filteredItems} columns={cols} getKey={i => i.id} emptyMessage="Nenhum tipo de equipamento encontrado." />
      </div>

      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? `Editar: ${editingItem.nome}` : "Novo Tipo de Equipamento"} className="max-w-md"
        ...
        <div className="space-y-4">
          <Input label={<span>Nome <span className="text-red-500">*</span></span>} placeholder="Ex: Caminhão" value={formData.nome} onChange={(e) => setFormData({ nome: e.target.value })} />
        </div>
      </Modal>
