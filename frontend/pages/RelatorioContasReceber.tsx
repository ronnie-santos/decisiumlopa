...
import { Search, X, FileDown, TrendingUp, CheckCircle, AlertCircle } from 'lucide-react';
...
// ── Component ─────────────────────────────────────────────────────────────────
export function RelatorioContasReceberPage() {
  ...
  const [filtrarCliente, setFiltrarCliente] = useState(false);
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      if (filtrarCliente  && clienteId)  params.set('idcliente', clienteId);
      if (filtrarEmpresa  && empresaId)  params.set('idempresa', empresaId);

      const res = await fetch(`/api/contas-receber/relatorio?${params}`);
      if (!res.ok) {
  ...
  const handleGerarPdf = () => {
    ...
    if (filtrarCliente && clienteId) params.set('idcliente', clienteId);
    if (filtrarEmpresa && empresaId) params.set('idempresa', empresaId);
    window.open(`/api/contas-receber/relatorio/pdf?${params}`, '_blank');
  };

  const totalAberto = data
    ? data.grupos.reduce((s, g) =>
        s + g.rows.filter(r => r.situacao === false).reduce((a, r) => a + r.valor, 0), 0)
    : 0;

  const qtdAberto = data
    ? data.grupos.reduce((s, g) => s + g.rows.filter(r => r.situacao === false).length, 0)
    : 0;

  return (
    <div className="flex flex-col h-full">
      <Header title="Relatório: Contas a Receber" />
      ...
      <div className="p-5 space-y-4">
        ...
        {data && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border-l-4 border-emerald-500 p-4 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Total a Receber</p>
            ...
            <div className="bg-white rounded-xl border-l-4 border-blue-500 p-4 shadow-sm flex items-center justify-between">
              ...
              <div className="h-10 w-10 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600 flex-shrink-0 ml-3">
            ...
            <div className="bg-white rounded-xl border-l-4 border-amber-400 p-4 shadow-sm flex items-center justify-between">
              <div>
                ...
                <h3 className="text-xl font-black text-slate-800">{brl(totalAberto)}</h3>
                <p className="text-[10px] text-slate-400 mt-0.5">
                  {qtdAberto} aberta{qtdAberto !== 1 ? 's' : ''}
                </p>
              </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm space-y-4">
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <Button variant="outline" className="gap-2" onClick={handleLimpar}>
              <X className="h-4 w-4" />
              Limpar
            </Button>
            {data && (
          ...
          <div className="flex flex-wrap gap-6 pt-1 border-t border-slate-50 items-end">
            ...
            <div className="flex flex-col gap-2">
              <Toggle
                ...
                onChange={v => { setFiltrarCliente(v); if (!v) setClienteId(''); }}
              ...
              <>
                <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2.5 bg-slate-800">
                    <span className="text-sm font-bold text-white uppercase tracking-wide">Contas a Receber</span>
                  ...
                  <div key={gi} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-800">
                      ...
                      <div className="flex items-center gap-4 text-xs">
                        <span className="text-slate-400">{g.count} conta{g.count !== 1 ? 's' : ''}</span>
                        <span className="font-bold text-emerald-300">{brl(g.subtotal_valor)}</span>
                        <span className="font-bold text-blue-300">{brl(g.subtotal_pago)} recebido</span>
                      </div>
                    </div>
                    <ContasTable rows={g.rows} />
                  </div>
                ))}
                <TotalBar totalValor={data.total_valor} totalPago={data.total_pago} totalRegistros={data.total_registros} />
              </>
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-slate-50/70 border-b border-slate-100">
            ...
            <th className="px-3 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ordens OS</th>
            ...
            <th className="px-3 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Valor Recebido</th>
          ...
          {rows.map((row, idx) => {
            ...
            return (
              <tr
                ...
                className={cn(
                  'transition-colors',
                  isAberta ? 'bg-amber-50/50 hover:bg-amber-50' : 'bg-green-50/20 hover:bg-slate-50/50'
                )}
              >
                ...
                <td className="px-3 py-2 text-xs text-slate-500 max-w-[160px]">
                  {row.ordens
                    ? <span className="font-mono text-[11px] text-slate-600">{row.ordens}</span>
    ...
    <div className="bg-slate-800 rounded-xl px-5 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        <div className="text-right">
          <p className="text-[10px] text-slate-500 uppercase tracking-widest">A Receber</p>
          <p className="text-sm font-black text-emerald-300">{brl(totalValor)}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-slate-500 uppercase tracking-widest">Recebido</p>
          <p className="text-sm font-black text-blue-300">{brl(totalPago)}</p>
        </div>
      </div>
    </div>
  );
