...
import { FornecedorAtividade } from '../types';
import { DataGrid, GridColumn } from '../components/ui/DataGrid';

export function AtividadeFornecedorPage() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<FornecedorAtividade | null>(null);
  const [items, setItems] = useState<FornecedorAtividade[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<FornecedorAtividade | null>(null);
  ...
  const filteredItems = items
    ...
    .sort((a, b) => Number(a.idatividade) - Number(b.idatividade));

  const cols: GridColumn<FornecedorAtividade>[] = [
    { header: 'Código', render: i => <span className="text-xs font-bold text-[#B21212]">{i.idatividade}</span> },
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch('/api/atividade-fornecedor');
      if (response.ok) {
        const data = await response.json();
        setItems(data.map((d: any) => ({ ...d, id: String(d.idatividade) })));
      } else {
        console.error('Falha ao carregar atividades');
      }
    } catch (error) {
  ...
  const handleOpenEditModal = (item: FornecedorAtividade) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isEditing ? `/api/atividade-fornecedor/${editingItem!.idatividade}` : '/api/atividade-fornecedor';
  ...
  const handleDeleteRequest = (item: FornecedorAtividade) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/atividade-fornecedor/${itemToDelete.idatividade}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchItems();
  ...
  return (
    <div className="flex flex-col h-full">
      <Header title="Atividades dos Fornecedores" />
      ...
      <div className="p-5 space-y-4">
        ...
        <DataGrid data={filteredItems} columns={cols} getKey={i => i.id} emptyMessage="Nenhuma atividade encontrada." />
      </div>
      ...
      <Modal
        ...
        title={editingItem ? `Editar: ${editingItem.descricao}` : "Nova Atividade"}
        className="max-w-md"
        footer={
          <>
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="Ex: Transporte de Cargas"
        ...
        <div className="flex flex-col items-center text-center space-y-4 py-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Deseja realmente excluir a atividade <span className="font-bold text-slate-700">{itemToDelete?.descricao}</span>?
