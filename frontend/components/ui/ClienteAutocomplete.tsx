...
export function ClienteAutocomplete({ value, displayName, onChange, disabled, placeholder }: Props) {
  const [query, setQuery] = useState('');
  const [suggestions, setSuggestions] = useState<ClienteOption[]>([]);
  ...
  // Sync display when value/name changes externally (e.g., opening edit modal)
  useEffect(() => {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`/api/clientes/options?nome=${encodeURIComponent(q)}`);
      if (r.ok) {
        const data: ClienteOption[] = await r.json();
    ...
    <div ref={containerRef} className="relative">
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder={placeholder ?? 'Buscar cliente por nome...'}
          value={query}
            ...
            <li key={c.idcliente}>
              ...
              >
                ...
                <div className="flex gap-3">
                  {c.nomefantasia && <span className="text-xs text-slate-400">{c.nomefantasia}</span>}
                  {c.cnpj_cpf && <span className="text-xs text-slate-400">{c.cnpj_cpf}</span>}
                </div>
              </button>
            </li>
          ))}
